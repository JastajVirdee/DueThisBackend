...
public class TestDueThisController {
    ...
    float gradeWeight = 0.4f;
    float newGradeWeight = 0.5f;
    ...
    public void testEditAssignmentNoviceStudent() {
        ...
        try {
            dtc.editAssignment(s.getAssignment(0), newName, newCourse, newDueDate, newGradeWeight,
                    newCompTime, s);
        } catch (InvalidInputException e) {
            fail();
        }
    ...
    public void testEditAssignmentNoviceInvalidInputs() {
        ...
        assertEquals(
                "Please enter a valid assignment name! Please enter a valid course name! Due date must be in the future! Please enter a positive grade weight! ",
                error);
    }
    ...
    public void testEditAssignmentExperiencedInvalidInputs() {
        ...
        assertEquals(
                "Please enter a valid assignment name! Please enter a valid course name! Due date must be in the future! Please enter an estimated completion time! ",
                error);
    }
    ...
    private Student createNoviceStudent(Application app) {
        Student s = new Student("testId1", "Richard Potato", "food", "a@b.c", false, 0, 0, 0, 0, 0,
                0, 0, app);
        return s;
    }

    private Student createExperiencedStudent(Application app) {
        Student s = new Student("testId", "Richard Potato", "food", "a@b.c", true, 5, 5, 5, 5, 5, 5,
                5, app);
        return s;
    }

    private Student createExperiencedStudent2(Application app) {
        Student s = new Student("UID", "Ichard Potato", "nope", "a@b.c", true, 5, 5, 5, 5, 5, 5, 5,
                app);
        return s;
    }

    private Assignment createAssignment(Student s, Application app) {
        return new Assignment("testId", name, course, dueDate, isCompleted, gradeWeight, compTime,
                s, app);
    }
    ...
    private Assignment createAssignmentDetailed(Student s, Date date, Application app) {
        return new Assignment("testId", name, course, date, isCompleted, gradeWeight, compTime, s,
                app);
    }

    private Assignment createAssignmentCourse(Student s, String newCourse, Application app) {
        return new Assignment("testId", name, newCourse, dueDate, isCompleted, gradeWeight,
                compTime, s, app);
    }
    ...
    private Assignment createAssignmentDetailedWithId(Student s, Date date, Application app,
            String id) {
